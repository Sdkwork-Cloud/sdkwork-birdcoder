  ...
  return (
    <div className="flex-1 overflow-y-auto p-12 bg-[#0e0e11]">
      <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 fill-mode-both" style={{ animationDelay: '0ms' }}>
        <h1 className="text-2xl font-semibold text-white mb-8">Personalization</h1>
        
        <div className="bg-[#18181b] rounded-xl border border-white/10 overflow-hidden mb-8">
          <div className="p-4 border-b border-white/10">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-white font-medium">Custom instructions</div>
                <div className="text-sm text-gray-500">Give AI extra context about your preferences and workflow.</div>
              </div>
              {isEditingInstructions ? (
                <div className="flex gap-2">
                  <Button variant="ghost" onClick={() => setIsEditingInstructions(false)}>Cancel</Button>
                  <Button variant="default" onClick={handleSaveInstructions}>Save</Button>
                </div>
              ) : (
            ...
            {isEditingInstructions ? (
              ...
              <div className="w-full bg-black/30 border border-white/5 rounded-lg p-3 text-sm text-gray-400 whitespace-pre-wrap min-h-[80px]">
                {settings.customInstructions || <span className="text-gray-600 italic">No custom instructions defined.</span>}
              </div>
            )}
          </div>
          <div className="flex items-center justify-between p-4">
            <div>
              <div className="text-white font-medium">Code snippet style</div>
              <div className="text-sm text-gray-500">Choose your preferred style for AI-generated code.</div>
            </div>
